...
import { storeDiscordTokens } from '../database/db';
...
export function getOAuthUrl() {
  ...
  url.searchParams.set('client_id', config.RP_CLIENT_ID);
  url.searchParams.set('redirect_uri', config.RP_REDIRECT_URI);
...
export async function getOAuthTokens(code) {
  ...
  const body = new URLSearchParams({
    client_id: config.RP_CLIENT_ID,
    client_secret: config.RP_CLIENT_SECRET,
...
export async function pushMetadata(userId, tokens, metadata) {
  ...
  const url = `https://discord.com/api/v10/users/@me/applications/${config.RP_CLIENT_ID}/role-connection`;
  const accessToken = await getAccessToken(userId, tokens);
  const body = {
    platform_name: 'regular.place',
    metadata,
  };
  const response = await fetch(url, {
    method: 'PUT',
